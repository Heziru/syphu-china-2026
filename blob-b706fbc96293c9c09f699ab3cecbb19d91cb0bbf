...
import type { LabChairStats } from "./createLabChairModel";
import referenceUrl from "./reference.png";
import { type LabChairReviewView } from "./reviewShots";

type Perf = { fps: number; triangles: number; calls: number };

type Props = {
  view: LabChairReviewView;
  onView: (view: LabChairReviewView) => void;
};

export function LabChairReviewHud({ view, onView }: Props) {
  const [stats, setStats] = useState<LabChairStats | null>(null);
  const [perf, setPerf] = useState<Perf | null>(null);

  useEffect(() => {
    const read = () => {
      const host = window as Window & {
        __LAB_CHAIR_STATS?: LabChairStats;
        __LAB_PERF?: Perf;
      };
      if (host.__LAB_CHAIR_STATS) setStats(host.__LAB_CHAIR_STATS);
  ...
  return (
    <aside className="lab-review">
      ...
      <img className="lab-review__image" src={referenceUrl} alt="Lab chair reference" />
