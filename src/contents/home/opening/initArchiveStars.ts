/**
 * Verbatim port of Nocturne-Memory-Core dashboard.html → initArchiveStars()
 * @see https://github.com/Pyruslili/Nocturne-Memory-Core/blob/main/dashboard.html
 */
export function initArchiveStars(canvas: HTMLCanvasElement): () => void {
  ...
  let frameTimer = 0;
  let lastStarFrame = 0;
  const targetFps = innerWidth < 900 ? 16 : 22;
  ...
  const makeStars = () => {
    const count = Math.max(
      120,
      Math.min(260, Math.round((innerWidth * innerHeight) / 7200)),
    );
  ...
  const drawStars = (time: number, loop = true) => {
    ...
    for (const star of stars) {
      if (!reduced) {
        star.x = (star.x + star.vx * dt + innerWidth) % innerWidth;
        star.y = (star.y + star.vy * dt + innerHeight) % innerHeight;
      }
