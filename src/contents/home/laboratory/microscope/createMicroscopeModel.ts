...
function createBaseLegacy(mats: Mats) {
  ...
  addMesh(
    group,
    extrudeY(roundedRect(0.54, 0.28, 0.045), 0.09, 0.01),
    mats.shell,
    "baseRear",
    [0, 0.03, -0.09],
  );
  addMesh(
    group,
    extrudeY(roundedRect(0.168, 0.3, 0.04), 0.09, 0.01),
    mats.shell,
    "baseToeL",
    [-0.186, 0.03, 0.155],
  );
  ...
  addMesh(
    group,
    extrudeY(roundedRect(0.56, 0.3, 0.04), 0.018, 0.004),
    mats.dark,
    "plateRear",
    [0, 0.008, -0.09],
  );
  addMesh(
    group,
    extrudeY(roundedRect(0.18, 0.312, 0.036), 0.018, 0.004),
    mats.dark,
    "plateToeL",
    [-0.186, 0.008, 0.155],
  );
...
function createArmLegacy(mats: Mats) {
  ...
  addMesh(
    group,
    extrudeY(roundedRect(0.13, 0.11, 0.024), 0.09, 0.008),
    mats.shell,
    "headBlock",
    [0, 0.81, 0.02],
  );
...
function createEyepieceLegacy(mats: Mats) {
  ...
  addMesh(
    ocular,
    new CylinderGeometry(0.033, 0.04, eyeH, 14),
    mats.dark,
    "ocularTube",
  );
  addMesh(
    ocular,
    new CylinderGeometry(0.044, 0.035, 0.02, 14),
    mats.dark,
    "eyeCup",
    [0, eyeH * 0.48, 0],
  );
...
function createTurretLegacy(mats: Mats) {
  ...
  addMesh(
    group,
    new CylinderGeometry(0.096, 0.104, 0.032, 16),
    mats.turret,
    "turretDisk",
    [0, cy, cz],
  );
  ...
  specs.forEach((spec, i) => {
    ...
    obj.position.set(
      Math.sin(spec.a) * 0.058,
      cy - 0.018,
      cz + Math.cos(spec.a) * 0.012,
    );
    addMesh(
      obj,
      new CylinderGeometry(0.02, 0.024, spec.len * 0.7, 12),
      mats.metal,
      "objBarrel",
      [0, -spec.len * 0.35, 0],
    );
    addMesh(
      obj,
      new CylinderGeometry(0.012, 0.014, 0.012, 10),
      mats.dark,
      "objNose",
      [0, -spec.len * 0.72, 0],
    );
...
function createBase(mats: Mats, style: Style) {
  ...
  addMesh(
    group,
    extrudeY(roundedRect(0.48, 0.36, 0.055), 0.088, 0.014),
    mats.shell,
    "baseShell",
    [0, 0.028, 0.02],
  );
  addMesh(
    group,
    extrudeY(roundedRect(0.5, 0.38, 0.05), 0.016, 0.004),
    mats.dark,
    "basePlate",
    [0, 0.006, 0.02],
  );
  ...
  (
    ...
    addMesh(group, i === 0 ? foot : foot.clone(), mats.dark, `foot${i}`, [
      xz[0],
      0.006,
      xz[1],
    ]);
...
function createArm(mats: Mats, style: Style) {
  ...
  addMesh(
    group,
    extrudeY(roundedRect(0.18, 0.12, 0.028), 0.1, 0.01),
    mats.shell,
    "headBlock",
    [0, 0.78, 0.04],
  );
...
function createEyepiece(mats: Mats, style: Style) {
  ...
  addMesh(
    group,
    extrudeY(roundedRect(0.16, 0.1, 0.022), 0.08, 0.008),
    mats.shell,
    "binocularHead",
    [0, 0.9, 0.08],
  );
  ...
  ([-1, 1] as const).forEach((side, i) => {
    ...
    addMesh(
      tube,
      new CylinderGeometry(0.028, 0.032, tubeLen, 12),
      mats.shell,
      "tube",
    );
    addMesh(
      tube,
      new CylinderGeometry(0.034, 0.03, 0.018, 12),
      mats.dark,
      "eyeCup",
      [0, tubeLen * 0.42, 0],
    );
    addMesh(
      tube,
      new CylinderGeometry(0.02, 0.02, 0.006, 10),
      mats.glass,
      "lens",
      [0, tubeLen * 0.52, 0],
    );
...
function createTurret(mats: Mats, style: Style) {
  ...
  addMesh(
    group,
    new CylinderGeometry(0.09, 0.098, 0.028, 16),
    mats.turret,
    "turretDisk",
    [0, cy, cz],
  );
  ...
  specs.forEach((spec, i) => {
    const obj = part(`objective${i}`);
    obj.position.set(
      Math.sin(spec.a) * ring,
      cy - 0.02,
      cz + Math.cos(spec.a) * 0.02,
    );
    addMesh(
      obj,
      new CylinderGeometry(0.018, 0.022, spec.len * 0.55, 10),
      mats.metal,
      "barrel",
      [0, -spec.len * 0.28, 0],
    );
    addMesh(
      obj,
      new CylinderGeometry(0.024, 0.024, 0.01, 10),
      mats.dark,
      "band",
      [0, -spec.len * 0.45, 0],
    );
    addMesh(
      obj,
      new CylinderGeometry(0.012, 0.016, spec.len * 0.28, 10),
      mats.metal,
      "tip",
      [0, -spec.len * 0.62, 0],
    );
...
function createStage(mats: Mats, style: Style) {
  ...
  const outline = roundedRect(
    style === "legacy" ? 0.36 : 0.34,
    style === "legacy" ? 0.27 : 0.24,
    0.02,
  );
  if (style === "concept") {
    const hole = new Path();
  ...
  addMesh(
    group,
    extrudeY(roundedRect(0.09, 0.08, 0.016), 0.11, 0.006),
    mats.shell,
    "stageBracket",
    [0, 0.42, -0.05],
  );
  ...
  addMesh(
    group,
    new CylinderGeometry(0.038, 0.044, 0.04, 12),
    mats.dark,
    "substage",
    [0, 0.455, 0.1],
  );
...
function createIlluminator(mats: Mats) {
  ...
  addMesh(
    group,
    new CylinderGeometry(0.048, 0.052, 0.03, 14),
    mats.dark,
    "lampBody",
    [0, 0.14, 0.06],
  );
  addMesh(
    group,
    new CylinderGeometry(0.034, 0.034, 0.008, 14),
    mats.light,
    "lampLens",
    [0, 0.158, 0.06],
  );
  addMesh(
    group,
    new CylinderGeometry(0.022, 0.022, 0.004, 12),
    mats.glass,
    "lampGlass",
    [0, 0.164, 0.06],
  );
...
export function createMicroscopeModel(
  options?: MicroscopeModelOptions,
): MicroscopeBuild {
