...
import type { BioreactorStats } from "./createBioreactorModel";
import referenceUrl from "./reference.png";
import { type BioreactorReviewView } from "./reviewShots";
...
type Props = {
  view: BioreactorReviewView;
  onView: (view: BioreactorReviewView) => void;
};

export function BioreactorReviewHud({ view, onView }: Props) {
  const [stats, setStats] = useState<BioreactorStats | null>(null);
  const [perf, setPerf] = useState<Perf | null>(null);

  useEffect(() => {
    const read = () => {
      const host = window as Window & {
        __BIOREACTOR_STATS?: BioreactorStats;
        __LAB_PERF?: Perf;
      };
      if (host.__BIOREACTOR_STATS) setStats(host.__BIOREACTOR_STATS);
  ...
  return (
    <aside className="lab-review">
      ...
      <img className="lab-review__image" src={referenceUrl} alt="Bioreactor reference" />
