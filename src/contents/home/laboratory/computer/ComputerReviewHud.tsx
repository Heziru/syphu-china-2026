...
import type { ComputerStats } from "./createComputerModel";
import referenceUrl from "./reference.png";
import { type ComputerReviewView } from "./reviewShots";
...
type Props = {
  view: ComputerReviewView;
  onView: (view: ComputerReviewView) => void;
};

export function ComputerReviewHud({ view, onView }: Props) {
  ...
  useEffect(() => {
    const read = () => {
      const host = window as Window & {
        __COMPUTER_STATS?: ComputerStats;
        __LAB_PERF?: Perf;
      };
      if (host.__COMPUTER_STATS) setStats(host.__COMPUTER_STATS);
  ...
  return (
    <aside className="lab-review">
      ...
      <img className="lab-review__image" src={referenceUrl} alt="Computer workstation reference" />
