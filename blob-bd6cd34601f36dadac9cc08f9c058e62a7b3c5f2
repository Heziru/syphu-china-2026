...
import type { AnalyticalBalanceStats } from "./createAnalyticalBalanceModel";
import referenceUrl from "./reference.png";
import { type AnalyticalBalanceReviewView } from "./reviewShots";
...
type Props = {
  view: AnalyticalBalanceReviewView;
  onView: (view: AnalyticalBalanceReviewView) => void;
};

export function AnalyticalBalanceReviewHud({ view, onView }: Props) {
  const [stats, setStats] = useState<AnalyticalBalanceStats | null>(null);
  ...
  useEffect(() => {
    const read = () => {
      ...
      if (host.__ANALYTICAL_BALANCE_STATS) setStats(host.__ANALYTICAL_BALANCE_STATS);
  ...
  return (
    <aside className="lab-review">
      ...
      <img className="lab-review__image" src={referenceUrl} alt="Analytical balance reference" />
